		...
		if args.__len__():
			...
			if user_id.startswith("<@") and user_id.endswith(">"):
				...
				try:
					...
					return await self.parent.send_message_w_fields(message, "Invalid user", "Use g!bal [@user]", discord.Color.red(), "Invalid argument @user", "Try mentioning a user")
...
class SlotMachine:
	...
	def __init__(self, parent: "gamble.GambleBot"):
		self.parent = parent
		self.slots: List[List[SlotElement]] = [[parent.command_handler.slots_manager.random_slot() for _ in range(3)] for _ in range(3)]
		self.slot_message: discord.Message = None

	def new_row(self):
		self.slots[1:3] = self.slots[0:2]
		self.slots[0] = [self.parent.command_handler.slots_manager.random_slot() for _ in range(3)]

	async def payout(self, author: discord.User):
		print("Payout")
	...
	async def roll(self, message: discord.Message):
		async def do_a_barrel_roll():
			...
			self.new_row()
			await self.slot_message.edit(content = self.__str__(), allowed_mentions = discord.AllowedMentions(replied_user = False))
...
class Slots(Command):
	...
	async def __call__(self, args: List[str], message: discord.Message, author: discord.User, guild: Optional[discord.Guild]) -> None:
		machine = SlotMachine(self.parent)
		await machine.roll(message)
		await machine.payout(author)
